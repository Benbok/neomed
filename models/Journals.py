...
class NeomedJournal(models.Model):
    ...
    def _get_default_day(self):
        my = self.date
        # birth_day = self.env['neomed.mothers'].browse(self._context['default_mother_id']).birth_day
        birth_day = self.mother_id.birth_day
        if not birth_day:
            return
        sample = my - birth_day
        if sample.days == 0:
            ...
            return {
                'warning': {
                    'title': "Ошибка",
                    'message': "В этот день ребенок еще не родился, вы уверены, что в этой ситуации дневник для него уместен?"
                },
            }
    ...
    #     # gv = self.env['neomed.mothers'].browse(self._context['default_mother_id']).GV
    ...
    def _get_default_gv(self):
        ...
        analizes = self.mother_id.analyzes_ids
        berem = self.mother_id.pregnancy_namber
        rod = self.mother_id.birth_namber
        fon = self.mother_id.anamnez
        ...
        day_now = self.date - self.mother_id.birth_day

        for rec in analizes:
            day_analyzes = rec.date - self.mother_id.birth_day
        ...
        if self.dopolnenie == 'желтушность':
            dop_text = "Кожные покровы розовые, иктеричные, по Крамеру () зона. " \
                       "Уровень билирубина, определенный транскутанным методом, составил (180)мкмоль/л"
        else:
            dop_text = "Кожные покровы розовые, чистые"
        ...
        if self.dopolnenie == 'искусственное вскармливание':
            ...
            dop_text_nutr = 'На смешанном вскармливании с докормом молочной смесью по ' + str(V_nutr1) + ' мл 4 -5 раз в день'
            nurt_rec = "смешанное вскармливание с докормом молочной смесью по " + str(V_nutr1) + ' мл 4 -5 раз в день' + " (расчет питания калорийным методом)"
        else:
            dop_text_nutr = 'На грудном вскармливании'

        sevir = ''
        if self.sevir == 'main1':
            sevir = "совместное пребывание"



        elif self.sevir == 'main2':
            sevir = "детское отделение"
        # %%%%%%%%%%%%%%%% конец дополнение к дневнику %%%%%%%%%%%%%%%%

        # ----------подготовка к обоснованию диагноза--------

        self.rec = '''{}
{}'''.format(nurt_rec, sevir)
...
Оценка физического развития:'''.format(dop_text_nutr, dop_text)
    ...
    @api.onchange('checkbox_OPN' , 'checkbox_vac' , 'checkbox_home')
    def _get_default_OPN(self):
        ...
        if self.checkbox_vac == True:
            self.rec += 'Вакцинаци БЦЖ м 0,025 разрешена\n'

        if self.checkbox_home == True:
            self.rec += 'Выписка домой. Рекомендации по дальнейшему ведению и уходу даны.'
    ...
    def _funct_time_check(self):
        for res in self:
            res.time_compute = res.time[:-3]
                    ...
                    # modelObj = self.mother_id.analyzes_ids
                    # for r in self:
                    #     for rec in modelObj.search([('number', '=', self.number), ('date', '=', self.date)]):
                    #         for i in rec.time:
                    #             if i[:-3] == r.time[:-3]:
                    #                 self.time_compute_check = True
                    #             self.time_compute = i
...
4)Кровь на ФКУ, ВГ, МВ, ГАО, АГС, 5)УЗИ органов грудной и брюшной полости""")
    ...
    rec = fields.Text(string='В плане: ')
    ...
    def _get_default_checkbox_degyrstvo(self):
        if self.checkbox_degyrstvo == True:
            self.dnevnic = "                                                                                    По дежурству \nДата   %s		Время  16ч  00мин  \n" \
                           "ЧСС: %sуд/мин	ЧД: %s/мин 	St 98 		Т 36,7 С \n" \
                           "%s\n" \
                           "                                                                                                                      Врач: %s   %s" % (self.date, self.ChSS, self.ChD, self.text, self.doc_otdel, self.time_compute)
        else:
            self.dnevnic = ''
